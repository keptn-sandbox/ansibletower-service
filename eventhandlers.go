...
// HandleGetSliTriggeredEvent handles get-sli.triggered events if SLIProvider == ansible-tower-service
...
func HandleGetSliTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.GetSLITriggeredEventData) error {
	...
	if data.GetSLI.SLIProvider != "ansible-tower-service" {
	...
	// Get SLI File from ansible-tower-service subdirectory of the config repo - to add the file use:
	//   keptn add-resource --project=PROJECT --stage=STAGE --service=SERVICE --resource=my-sli-config.yaml  --resourceUri=ansible-tower-service/sli.yaml
	sliFile := "ansible-tower-service/sli.yaml"
...
func HandleActionTriggeredEvent(myKeptn *keptnv2.Keptn, incomingEvent cloudevents.Event, data *keptnv2.ActionTriggeredEventData) error {
	...
	if data.Action.Action == "job_template_launch" {
		...
		actionStartedEventData := &keptnv2.ActionStartedEventData{}
